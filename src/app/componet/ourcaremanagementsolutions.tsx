...
const Ourcaremanagementsolutions: React.FC = () => {
    ...
    useEffect(() => {
        const fetchPosts = async () => {
            const response: any = await client.getAllByType("ourcaremanagementsolutions");
            setPosts(response);
        };
                                    ...
                                    <Grid container spacing={0} key={post.id}>
                                        <Grid item lg={12} style={{ display: 'flex', justifyContent: 'center', flexDirection: 'row', gap: '20px' }}>
                                            <Grid item lg={4} style={{}}>
                                                {posts.map((post: any) => (
                                                    <Typography style={{ fontFamily: 'Mulish', fontWeight: 700, fontSize: '50px', lineHeight: '62.5px', color: '#292F36', letterSpacing: '2%' }}>{post.data.leftheader}</Typography>
                                                ))
                                                }
                                                <Typography style={{ fontFamily: 'Jost', color: '#4D5053', fontWeight: 400, fontSize: '22px', lineHeight: '33px', letterSpacing: '1%' }}>
                                                ...
                                                <div style={{ paddingTop: '68.14px', paddingBottom: '159.85px' }}>
                                                    {posts.map((post: any) => (
                                                        <Button style={{ display: 'flex', flexDirection: 'row', background: '#24535C', padding: '16px 24px 16px 24px', gap: '18px', borderRadius: '82px' }}>
                                                            ...
                                                            <img src={post.data.buttonimage.url || ''} alt={post.data.buttonimage} style={{ background: '#82C5BE', borderRadius: '15.9px', padding: '7.29px', width: '9.27px', height: '9.27px' }} />
                                                        </Button>
                                                    ))
                                                    }
                                                </div>
                                            ...
                                            <Grid item lg={4}>
                                                <img src={post.data.rightimage.url || ''} alt={post.data.rightimage} style={{ height: '700px', width: '653px' }} />
                                            </Grid>
